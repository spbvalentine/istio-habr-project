...
```
curl --cacert ca.crt -X POST -d "client_id=habr&client_secret=habr&grant_type=password&username=admin&password=admin" "https://$KEYCLOAK_HOST/auth/realms/habr/protocol/openid-connect/token" | jq -r '.access_token'
```
